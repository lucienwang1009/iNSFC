...
随着人工智能技术的迅速发展，数据生成问题日益受到研究者的广泛关注\upcite{胡铭菲2022深度生成模型综述,harshvardhan2020comprehensive}。
在众多数据生成的问题中，由于结构数据在现实世界与计算机科学领域的广泛存在，其生成技术可广泛应用于生物信息学\upcite{guo2024diffusion,vignac2023digress}、网络可靠性分析\upcite{duenas2017counting}、代码自动化测试\upcite{chakraborty_parallel_2015,soos_tinted_2020}以及机器学习模型的评估\upcite{yoon2023graph}等领域，具有重要的理论意义和应用价值，已逐渐成为研究的重点之一\upcite{guo2022systematic}。
% 成为研究的重点之一\upcite{guo2022systematic,codd1970relational}。
结构数据生成是指根据给定的数据分布，生成符合该分布且严格满足元素间结构约束的数据。
鉴于结构数据生成问题中的约束往往呈现出量化规律性，采用\emph{一阶逻辑}来描述这些结构约束显得尤为自然。
同时，在结构数据推理领域，一阶逻辑凭借其表达的简洁性、普适性，以及对高效求解算法的固有支持，已成为解决该类问题的标准范式之一\upcite{richardson_markov_2006,de_raedt_problog_2007,van_den_broeck_lifted_2011,van_den_broeck_lifted_2015,marra_neural_2021,van_bremen_lifted_2021,toth_lifted_2022}。
...
% 可抽象为一个加权一阶逻辑模型（注意此处为布尔变量的赋值）采样（Weighted First-Order Model Sampling, WFOMS）问题。
% 结构数据是指数据元素间存在严格约束的数据，并且这些约束能够通过形式化语言进行描述\upcite{codd1970relational,dvzeroski2010relational}。
% 此类问题的主要挑战在于：\emph{生成的结构数据不仅要满足特定的分布，还要}。
...
对一阶逻辑模型采样问题的研究在过去十几年中取得了一些进展\upcite{niepert_lifted_2012,niepert_markov_2012,holtzen_generating_2019,den_broeck_and_mathias_niepert_lifted_2015,gogate_advances_2012}，但一直缺乏系统的理论框架和高效的解决方案。
